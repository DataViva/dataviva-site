...
'''
class IndustyByLocationOcupation:
    def __init__(self, bra_id, cnae_id):
        self.bra_id = bra_id
        self.cnae_id = cnae_id
        self._rais = None
...
class Industry :
    ...
    def __rais_values__(self):
        if not self.industry_headers  :
            headers_obj = Yi.query.filter(
                Yi.cnae_id == self.cnae_id,
                Yi.year == self.yi_max_year_br    
                ).limit(1).first_or_404()
        
            self.industry_headers['average_monthly_income'] = headers_obj.wage_avg
            self.industry_headers['salary_mass'] = headers_obj.wage
            self.industry_headers['num_jobs'] = headers_obj.num_jobs
            self.industry_headers['num_establishments'] = headers_obj.num_est
...
class IndustryByLocation(Industry) :
    ...
    def __rais_values__(self):
        if not self.industry_headers : 
            ...
            self.industry_headers['num_jobs'] = headers_obj.num_jobs
            self.industry_headers['num_establishments'] =  headers_obj.num_est
            self.industry_headers['rca'] =  headers_obj.rca
            self.industry_headers['distance'] =  headers_obj.distance
            self.industry_headers['opportunity_gain'] =  headers_obj.opp_gain          
