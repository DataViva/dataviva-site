...
var ProductsTable = function () {
    ...
    this.table = $(this.tableId).DataTable({
        ...
        initComplete: function () {
            ...
            $('#products-postions').addClass('active');


            $('#products-sections').click(function() {
                loadingRankings.show();
                products.table.ajax.url("/attrs/hs?depht=2").load(loadingRankings.hide);
                $(this).addClass('active').siblings().removeClass('active');
            });

            $('#products-postions').click(function() {
                loadingRankings.show();
                products.table.ajax.url("/attrs/hs?depht=6").load(loadingRankings.hide);
                $(this).addClass('active').siblings().removeClass('active');
            });
