...
def index(cnae_id):
    ...
    if bra_id:
        ybi_max_year = db.session.query(func.max(Ybi.year)).filter_by(cnae_id=cnae_id, bra_id=bra_id)
        
        list_rais = Ybi.query.filter(
            Ybi.year == ybi_max_year,
            Ybi.bra_id == bra_id,
            Ybi.cnae_id_len == func.length(cnae_id)).order_by(desc(Ybi.num_jobs)).all()
        ...
        list_rais = Yi.query.filter(
            Yi.year == yi_max_year,
            Yi.cnae_id_len == func.length(cnae_id)).order_by(desc(Yi.num_jobs)).all()
    ...
    return render_template('industry/index.html', header=header, body=body, industry=industry)
