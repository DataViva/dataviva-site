...
def add_to_db(table):
    ...
    for attr_type in reversed(attrs[1:]):
        ...
        for aggregation_level in reversed(attr_aggs[attr_type][:-1]):
            ...
            if attr_name == "bra_id" and aggregation_level == 7:
                for plr_region in planning_regions:
                    ...
                    sql = "select {0}, sum(wage) as wage, "\
                            "sum(num_emp) as num_emp, sum(num_est) as num_est "\
                            "from {1} where bra_id in "\
                    ...
                    insert_columns_str = ", ".join(columns + ["wage", "num_emp", "num_est"])
                    insert_sql = "insert into {0} ({1}) {2}"\
                                    .format(table, insert_columns_str, sql)
                    
                    print insert_sql; print
