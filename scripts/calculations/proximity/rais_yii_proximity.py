...
def delete_old_proximity_from_db(year):
    ...
    cursor.execute("delete from rais_yii where year=%s ", (year))
...
def add_new_proximity_to_db(year, prox):
    ...
    counter = 0
    total_inserts = 0
    for isic in prox.columns:
        for isic_2 in prox.index:
            
            '''dont insert null values'''
            if prox[isic][isic_2] == 0 or prox[isic][isic_2] is None:
                continue
            
            counter += 1
            total_inserts += 1
            to_add.append([year, isic, isic_2, prox[isic][isic_2]])
            if counter >= 2500:
                cursor.executemany("insert into rais_yii values " \
                                "(%s, %s, %s, %s)", to_add)
                counter = 0
                to_add = []
    ...
    cursor.executemany("insert into rais_yii values (%s, %s, %s, %s)", to_add)
