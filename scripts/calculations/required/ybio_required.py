...
def get_ybio_panel(**kwargs):
    q = """
        select 
          ybio.bra_id, isic_id, cbo_id, num_emp 
        from 
          rais_ybio as ybio
        where 
          ybio.year = {0} and 
...
def main(**kwargs):
    ...
    ybio_panel = get_ybio_panel(year=kwargs["year"], 
                    ...
                    pop_cutoff=kwargs["pop_cutoff"], val="num_emp")
    # print ybio_panel["mg030000"].ix["s9492"].dropna()[ybio_panel["mg030000"].ix["s9492"]> 0]; sys.exit()
    ...
    for geo in rcas.index:
        ...
        # maybe use YB
        # this_state_emps = mcp_num_emp.ix[geo:geo].reindex(mcp_num_emp.index, method="ffill")
        ...
        # print this_state_emps.ix["mg"]
        # sys.exit()
        best_matches = mcp_num_emp.corrwith(this_state_emps, axis=1).fillna(0)
        avg_ranks = (best_matches.rank(ascending=False) + prox[geo].rank(ascending=False)) / 2
        avg_ranks = avg_ranks.dropna()
        ...
        # add to db
        cursor.executemany("INSERT INTO rais_ybio (year, bra_id, isic_id, cbo_id, required) VALUES (%s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE required=%s;", to_add)
