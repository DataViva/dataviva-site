...
def delete_old_opp_gain_from_db(year, geo_level):
    ...
    q = "update rais_ybi set opp_gain=NULL " \
...
def add_new_opp_gain_to_db(year, opp_gain):
    ...
    for hs in opp_gain.columns:
        for bra in opp_gain.index:
            ...
            if counter >= 2500:
                cursor.executemany("update rais_ybi set opp_gain=%s where " \
                                "year=%s and bra_id=%s and isic_id=%s", to_add)
                counter = 0
                to_add = []
                
            if total_inserts % 10000 == 0:
                sys.stdout.write('\r rows updated: ' + str(total_inserts) \
                                    + ' ' * 20)
...
def calculate_opp_gain(year, geo_level):
    ...
    rca = rais_ybi_rca.get_rca(year, geo_level)
