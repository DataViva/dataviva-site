...
if [[ $# != 3 ]]; then
    echo "Error, use:"
    echo "./scripts/data_download/gera_rais.sh en/pt output_path year &> out scripts/data_download/out_rais_years.txt&"
...
for num in  {1..4}; 
    do echo $1 $2 $year $3;
    python scripts/data_download/rais.py $1 $2 $3 $num
    sleep 5
done
